...
**What I'd do differently with more time** is build the eval pipeline before shipping, and be precise about what kind of metric each number is. Missed-escalation rate is a *guardrail* — target ~0%, never traded off for a better average elsewhere. Escalation precision, category-match precision, and resolution rate are normal *quality* metrics, expected to move as the system tunes. Conflating the two is how a safety layer quietly gets weakened for a cleaner dashboard. Full method and worked examples in [`EVAL-METHODOLOGY.md`](https://github.com/emrefisek/SymptomSearch/blob/main/EVAL-METHODOLOGY.md).
