...
**The safety number is the one that matters most, and it fails.** A guardrail metric target of "as close to 0% miss rate as measurable" was set in `DESIGN-DECISIONS.md` §12. A 50% miss rate on held-out adversarial phrasing is nowhere near that target — the near-miss heuristic added earlier in this project (§2 of `DESIGN-DECISIONS.md`) genuinely fixed the three specific phrasings it was built to fix, and this eval proves it generalizes to *some* new phrasing (S1, S3, S4, S8 weren't in the original fix set either) — but it does **not** prove the underlying problem is solved. That's exactly the distinction an eval is for: without running this golden set, "we added a near-miss check" reads like a fix; with it, it reads correctly as *a partial, measured improvement with a known remaining gap*. Concrete next iteration, informed directly by the misses: allow noun+noun co-occurrence (not just noun+modifier) to trigger escalation, and expand the modifier list to include common causal/uncertainty phrasing ("attack," "lose," "a little") rather than only intensity words.

**The HWG-adjacent number is the sharpest finding in this whole exercise.** Zero of four. The near-miss pattern built for physical red-flags (§2) was never extended to the structurally identical problem for cancer- and addiction-adjacent phrasing — and H1 is the most concrete failure mode possible: a query about a possibly cancerous skin spot lands, by coincidence, in exactly the bucket that would recommend a fragrance-free moisturizer for it. This is not a hypothetical concern raised in a design document; it's a reproducible, demonstrated gap in the current build. The fix is the same *shape* as the safety fix: a second risk-cluster check (words like "cancer," "tumor," "mole"/"spot" + "changed"/"bigger"/"dark"; "drink" + "can't stop"/"every day") that flags regardless of exact-phrase overlap — but it needs to be built and evaluated separately, because building one near-miss net does not automatically extend coverage to a different sensitive-topic category.
...
It cannot answer a different, equally important question: **is this feature good for the business?** That requires **online eval** — real users, real traffic, an A/B test against the current keyword-search baseline — because things like conversion delta, the 7–14 day return-rate signal mentioned in `DESIGN-DECISIONS.md` §11, and actual escalation *precision* (as opposed to recall) can only be measured against real behavior, not a hand-labeled set. The two are complementary, not substitutes: a feature can pass every offline guardrail and still be a bad idea commercially, and it can be a great commercial idea while still having a golden-set-detectable safety bug. Ship only after both have a defensible answer.
...
| Return-within-14-days after referral/escalation | Commercial (online only) | High (signals the redirect was correct) | Needs real traffic |
